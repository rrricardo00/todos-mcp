...
export default function TodoList() {
  const { todos, loading, error: todosError, addTodo, updateTodo, removeTodo } = useTodos()
  const [error, setError] = useState<string | null>(null)
  const [activeTab, setActiveTab] = useState<TodoTab>('active')
  ...
  // Função para agrupar e ordenar todos por dia
  const groupedTodos = useMemo(() => {
    const filtered = todos.filter(todo => 
      activeTab === 'active' ? !todo.checked : todo.checked
    )
  ...
  const activeTodosCount = todos.filter(t => !t.checked).length
  const completedTodosCount = todos.filter(t => t.checked).length
          ...
          {Object.keys(groupedTodos).length === 0 ? (
            <div className="text-center py-16 bg-white rounded-xl border border-slate-200">
              ...
              <p className="text-slate-500 text-lg">
                {activeTab === 'active' ? 'Nenhum todo ativo' : 'Nenhum todo concluído'}
              </p>
              <p className="text-slate-400 mt-2">
                {activeTab === 'active' 
                  ? 'Adicione um novo todo acima para começar!' 
                  : 'Complete alguns todos para vê-los aqui!'}
              </p>
            </div>
          ) : (
            <div className="space-y-6">
              {Object.entries(groupedTodos).map(([dateKey, todosForDate], dateIndex) => (
                <div key={dateKey} className="space-y-4">
                  <h3 className="text-lg font-semibold text-slate-700 flex items-center gap-2 sticky top-0 bg-slate-50 py-2 z-10">
